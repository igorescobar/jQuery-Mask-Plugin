...
d),b.moveStart("character",d),b.select())},events:function(){a.on("keydown.mask",function(){n=f.val()});a.on("keyup.mask",f.behaviour);a.on("paste.mask",function(){setTimeout(function(){a.keydown().keyup()},100)})},destroyEvents:function(){a.off("keydown.mask").off("keyup.mask").off("paste.mask")},val:function(d){var b="input"===a.get(0).tagName.toLowerCase();return 0<arguments.length?b?a.val(d):a.text(d):b?a.val():a.text()},behaviour:function(a){a=a||window.event;if(-1===e.inArray(a.keyCode||a.which,
k.byPassKeys)){var b,c=f.getCaret();c<f.val().length&&(b=!0);f.val(f.getMasked());b&&f.setCaret(c);return f.callbacks(a)}},getMasked:function(a){var b=[],e=f.val(),h=0,q=g.length,l=0,t=e.length,m=1,u="push",r=-1,p,v;c.reverse?(u="unshift",m=-1,p=0,h=q-1,l=t-1,v=function(){return-1<h&&-1<l}):(p=q-1,v=function(){return h<q&&l<t});for(;v();){var w=g.charAt(h),x=e.charAt(l),s=k.translation[w],n=g.charAt(p);if(s)x.match(s.pattern)?(b[u](x),s.recursive&&(-1===r?r=h:h===p&&(h=r-m),p===r&&(h-=m)),h+=m):s.optional&&
(h+=m,l-=m),l+=m;else{if(void 0===a)b[u](w);x===w&&(l+=m);h+=m}}q!==t+1||k.translation[n]||b.push(n);return b.join("")},callbacks:function(d){var b=f.val(),e=f.val()!==n;if(!0===e&&"function"===typeof c.onChange)c.onChange(b,d,a,c);if(!0===e&&"function"===typeof c.onKeyPress)c.onKeyPress(b,d,a,c);if("function"===typeof c.onComplete&&b.length===g.length)c.onComplete(b,d,a,c)}};k.remove=function(){f.destroyEvents();f.val(k.getCleanVal()).removeAttr("maxlength")};k.getCleanVal=function(){return f.getMasked(!0)};
