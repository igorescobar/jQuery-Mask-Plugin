...
c-f,e):b.caretPos(a+1,c,e,f)},behaviour:function(a){a=a||window.event;b.invalid=[];var e=a.keyCode||a.which;if(-1===c.inArray(e,g.byPassKeys)){var d=b.getCaret(),f=b.val(),n=f.length,k=d<n,s=b.getMasked(),l=s.length,p=b.getMCharsBeforeCount(l-1)-b.getMCharsBeforeCount(n-1);s!==f&&b.val(s);!k||65===e&&a.ctrlKey||(8!==e&&46!==e&&(d=b.caretPos(d,n,l,p)),b.setCaret(d));return b.callbacks(a)}},getMasked:function(a){var c=[],p=b.val(),f=0,n=d.length,k=0,s=p.length,l=1,m="push",r=-1,u,w;e.reverse?(m="unshift",
l=-1,u=0,f=n-1,k=s-1,w=function(){return-1<f&&-1<k}):(u=n-1,w=function(){return f<n&&k<s});for(;w();){var x=d.charAt(f),v=p.charAt(k),t=g.translation[x];if(t)v.match(t.pattern)?(c[m](v),t.recursive&&(-1===r?r=f:f===u&&(f=r-l),u===r&&(f-=l)),f+=l):t.optional?(f+=l,k-=l):t.fallback?(c[m](t.fallback),f+=l,k-=l):b.invalid.push({p:k,v:v,e:t.pattern}),k+=l;else{if(!a)c[m](x);v===x&&(k+=l);f+=l}}a=d.charAt(u);n!==s+1||g.translation[a]||c.push(a);return c.join("")},callbacks:function(c){var g=b.val(),m=g!==
...
c.maskWatchers[e]=setInterval(function(){c(document).find(e).each(m)},p))};c.fn.unmask=function(){clearInterval(c.maskWatchers[this.selector]);delete c.maskWatchers[this.selector];return this.each(function(){c(this).data("mask")&&c(this).data("mask").remove().removeData("mask")})};c.fn.cleanVal=function(){return this.data("mask").getCleanVal()};var r=c.jMaskGlobals={nonInput:"td,span,div",dataMaskAttr:"*[data-mask]",dataMask:!0,watchInterval:300,watchInputs:!0,watchDataMask:!1,byPassKeys:[9,16,17,
18,36,37,38,39,40,91],translation:{0:{pattern:/\d/},9:{pattern:/\d/,optional:!0},"#":{pattern:/\d/,recursive:!0},A:{pattern:/[a-zA-Z0-9]/},S:{pattern:/[a-zA-Z]/}}};r.dataMask&&c(r.dataMaskAttr).each(A);r.watchDataMask&&setInterval(function(){c(document).find(r.nonInput).filter(maskAttr).each(A)},interval)});
