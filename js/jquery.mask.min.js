...
a=b.get(0).createTextRange();a.collapse(!0);a.moveEnd("character",r);a.moveStart("character",r);a.select()}}catch(c){}},events:function(){b.on("keydown.mask",function(a){b.data("mask-keycode",a.keyCode||a.which)}).on(a.jMaskGlobals.useInput?"input.mask":"keyup.mask",c.behaviour).on("paste.mask drop.mask",function(){setTimeout(function(){b.keydown().keyup()},100)}).on("change.mask",function(){b.data("changed",!0)}).on("blur.mask",function(){n===c.val()||b.data("changed")||b.triggerHandler("change");
b.data("changed",!1)}).on("blur.mask",function(){n=c.val()}).on("focus.mask",function(b){!0===d.selectOnFocus&&a(b.target).select()}).on("focusout.mask",function(){d.clearIfNotMatch&&!k.test(c.val())&&c.val("")})},getRegexMask:function(){for(var b=[],a,c,f,d,h=0;h<e.length;h++)(a=g.translation[e.charAt(h)])?(c=a.pattern.toString().replace(/.{1}$|^.{1}/g,""),f=a.optional,(a=a.recursive)?(b.push(e.charAt(h)),d={digit:e.charAt(h),pattern:c}):b.push(f||a?c+"?":c)):b.push(e.charAt(h).replace(/[-\/\\^$*+?.()|[\]{}]/g,
